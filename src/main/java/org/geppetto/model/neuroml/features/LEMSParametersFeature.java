...
public class LEMSParametersFeature implements ISetParameterFeature
{
	...
	public void setParameter(VariableValue variableValue) throws ModelInterpreterException
	{
		...
		Component component;
		if(PointerUtility.getType(variableValue.getPointer()).getDomainModel() != null)
		{
			component = (Component) PointerUtility.getType(variableValue.getPointer()).getDomainModel().getDomainModel();
		}
		else
		{
			component = (Component) variableValue.getPointer().getElements().get(variableValue.getPointer().getElements().size() - 2).getType().getDomainModel().getDomainModel();
		}

		String value = String.valueOf(((Quantity) variableValue.getValue()).getValue());

		component.setParameter(paramName, value);
